...
ExecutionBackend = Literal["eager", "cuda-graph"]
ProfilerBackend = Literal["auto", "eager", "cuda-graph"]
...
def prepare_profile_inputs(
    ...
    *,
    ...
    """Create one deterministic NO_CORR source/target pair on CUDA."""
    if not 0 <= source_index < sequence.points.shape[0] - 1:
        raise ValueError(
            "--profile-source-index must address a valid frame pair; "
            f"received {source_index}."
        )

    source_full = sequence.points[source_index]
    target_full = sequence.points[source_index + 1]
    ...
    source_indices = source_rng.choice(
        source_full.shape[0],
        size=num_points,
        replace=source_full.shape[0] < num_points,
    )
    target_indices = target_rng.choice(
        target_full.shape[0],
        size=num_points,
        replace=target_full.shape[0] < num_points,
    )

    source_np = np.ascontiguousarray(
        source_full[source_indices],
        dtype=np.float32,
    )
    target_np = np.ascontiguousarray(
        target_full[target_indices],
        dtype=np.float32,
    )

    source = torch.from_numpy(source_np).unsqueeze(0).to(device)
    target = torch.from_numpy(target_np).unsqueeze(0).to(device)
    return source, target


def profile_difflow3d(
    ...
    *,
    ...
    with_stack: bool,
    with_flops: bool,
    backend: Literal["eager", "cuda-graph"] = "eager",
    graph_runner: "CudaGraphDifFlowRunner | None" = None,
) -> None:
    """Profile eager model execution or CUDA-graph replay.

    Eager profiling retains the nested model ranges and is intended for
    bottleneck attribution. CUDA-graph profiling measures the actual static
    deployment path: input copies followed by one graph replay.
    """
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for DifFlow3D profiling.")
    if source.device.type != "cuda":
        raise ValueError("source must already be on CUDA.")
    if target.device != source.device:
        raise ValueError("source and target must use the same CUDA device.")
    if source.shape != target.shape:
        raise ValueError(
            f"source/target shapes differ: {source.shape} vs {target.shape}."
        )
    if source.ndim != 3 or source.shape[-1] != 3:
        raise ValueError(
            f"Expected source/target shape [B,N,3], got {source.shape}."
        )
    if backend not in ("eager", "cuda-graph"):
        raise ValueError(f"Unsupported profiler backend: {backend}.")
    if backend == "cuda-graph" and graph_runner is None:
        raise ValueError("CUDA-graph profiling requires graph_runner.")

    for name, value, minimum in (
        ("warmup_iterations", warmup_iterations, 0),
        ("wait_steps", wait_steps, 0),
        ("schedule_warmup_steps", schedule_warmup_steps, 0),
    ...
    def run_once() -> Any:
        if backend == "cuda-graph":
            ...
            graph_runner.copy_gpu_inputs(
                source,
                target,
                inverse_dt=1.0,
            )
            graph_runner.replay()
            return graph_runner.static_model_output
    ...
    # Warm up outside the profiler. The graph itself is already captured, but
    # replays are still warmed to stabilize clocks and allocator activity.
    ...
    def trace_handler(profiler) -> None:
        ...
        averages = profiler.key_averages(
            group_by_input_shape=record_shapes,
        )
        self_cuda_table = averages.table(
            sort_by="self_cuda_time_total",
            row_limit=row_limit,
        )
        total_cuda_table = averages.table(
        ...
        print("\n" + "=" * 120)
        print(
            f"PyTorch profiler ({backend}) — operators by self CUDA time"
        )
        print("=" * 120)
        print(self_cuda_table)
        print("\n" + "=" * 120)
        print(
            f"PyTorch profiler ({backend}) — ranges/operators by total CUDA time"
        )
        print("=" * 120)
        print(total_cuda_table)

        stem = f"cycle_{trace_counter:02d}_{backend.replace('-', '_')}"
        self_path = output_path / f"{stem}_self_cuda.txt"
        total_path = output_path / f"{stem}_total_cuda.txt"
        trace_path = output_path / f"{stem}_trace.json"

        self_path.write_text(self_cuda_table, encoding="utf-8")
        total_path.write_text(total_cuda_table, encoding="utf-8")
        profiler.export_chrome_trace(str(trace_path))

        print(f"\nSaved operator table: {self_path}")
        print(f"Saved range table:    {total_path}")
    ...
    total_steps = (
        wait_steps + schedule_warmup_steps + active_steps
    ) * repeat
            ...
            for _ in range(total_steps):
                if backend == "cuda-graph":
                    ...
                    with record_function(
                        "difflow3d/cuda_graph_input_copy"
                    ):
                        graph_runner.copy_gpu_inputs(
                            source,
                            target,
                    ...
                    with record_function(
                        "difflow3d/cuda_graph_replay"
                    ):
                        graph_runner.replay()
                else:
                    with record_function("difflow3d/full_forward"):
                        _ = run_once()
                prof.step()

    torch.cuda.synchronize(source.device)
    print(f"\nProfiler results saved under: {output_path}")
...
class CudaGraphDifFlowRunner:
    """Static-shape CUDA Graph runner for DifFlow3D inference.

    The captured graph includes the model forward and the inexpensive output
    post-processing (full-resolution flow layout, warping, and velocity).
    Input transfers remain outside the graph because their values change every
    frame.  The runner is shape-specialized to [1, num_points, 3].
    """
    ...
    def __init__(
        ...
        *,
        ...
        if device.type != "cuda":
            raise ValueError("CUDA Graph runner requires a CUDA device.")
        if capture_warmup < 1:
        ...
        self.static_target = torch.empty_like(self.static_source)
        self.static_dummy_flow = torch.zeros_like(self.static_source)
        self.static_inverse_dt = torch.ones(
            (),
            device=device,
            dtype=torch.float32,
        )
        ...
        self.static_model_output: Any = None
        self.static_predicted_flow: torch.Tensor
        self.static_warped_points: torch.Tensor
        self.static_velocity: torch.Tensor

        self._capture()

    def _model_forward(self) -> Any:
        return self.model(
            self.static_source,
            self.static_target,
            self.static_source,
            self.static_target,
            self.static_dummy_flow,
    ...
    def _capture(self) -> None:
        ...
        # Non-degenerate warmup values avoid exercising an unusual all-zero FPS
        # path while cuDNN/cuBLAS and custom kernels initialize.
        self.static_source.normal_(mean=0.0, std=0.25)
        self.static_target.normal_(mean=0.0, std=0.25)
        ...
        with torch.cuda.stream(warmup_stream):
            with torch.inference_mode():
                for _ in range(self.capture_warmup):
                    ...
                    _ = self.static_source[0] + predicted
                    _ = predicted * self.static_inverse_dt

        current_stream.wait_stream(warmup_stream)
        torch.cuda.synchronize(self.device)

        # Release Python references from warmup before capture.  The CUDA
        # caching allocator retains usable blocks.
        del output, predicted, _
        gc.collect()

        with torch.cuda.graph(self.graph):
            with torch.inference_mode():
                self.static_model_output = self._model_forward()
                self.static_predicted_flow = (
                    self.static_model_output[0][0][0]
                    .permute(0, 2, 1)[0]
                    .contiguous()
                )
                self.static_warped_points = (
                    self.static_source[0]
                    + self.static_predicted_flow
                )
                self.static_velocity = (
                    self.static_predicted_flow
                    * self.static_inverse_dt
                )

        # Ensure capture finished before the first caller writes input buffers.
        torch.cuda.synchronize(self.device)
    ...
    def copy_gpu_inputs(
        ...
        *,
        ...
        expected = self.static_source.shape
        if source.shape != expected or target.shape != expected:
            raise ValueError(
                "CUDA Graph input shape mismatch: "
                f"expected {expected}, got {source.shape}/{target.shape}."
            )
        self.static_source.copy_(source, non_blocking=True)
        self.static_target.copy_(target, non_blocking=True)
        self.static_inverse_dt.fill_(float(inverse_dt))

    def copy_pinned_inputs(
        self,
        source: torch.Tensor,
        target: torch.Tensor,
        *,
        ...
        if not source.is_pinned() or not target.is_pinned():
            raise ValueError("Host graph inputs must be pinned tensors.")
        self.static_source.copy_(source, non_blocking=True)
        self.static_target.copy_(target, non_blocking=True)
        self.static_inverse_dt.fill_(float(inverse_dt))
...
class DifFlow3DConfig:
    ...
    execution_backend: ExecutionBackend = "eager"
    cuda_graph_warmup: int = 5
    cuda_graph_fallback: bool = True
    cuda_graph_clone_outputs: bool = False
...
class DifFlow3DInference:
    ...
    def __init__(self, config: DifFlow3DConfig) -> None:
        ...
        self.requested_execution_backend = config.execution_backend
        self.execution_backend: ExecutionBackend = "eager"
        self.cuda_graph_error: str | None = None
        self.graph_runner: CudaGraphDifFlowRunner | None = None
        self._source_pinned: torch.Tensor | None = None
        self._target_pinned: torch.Tensor | None = None
        self._source_pinned_numpy: np.ndarray | None = None
        self._target_pinned_numpy: np.ndarray | None = None
        ...
        if self.num_points < 2048:
            raise ValueError(
                "DifFlow3D downsamples its first level to 2048 points; "
                "--difflow-num-points must be at least 2048."
            )
        if config.iters < 1:
        ...
        if config.execution_backend not in ("eager", "cuda-graph"):
            raise ValueError(
        ...
        if config.torch_compile_mode not in (
            "none",
            "default",
            "reduce-overhead",
            "max-autotune",
        ...
        module_name = config.model_module.strip()
        if not module_name:
            raise ValueError("model_module must not be empty.")
        module_file = repo_path.joinpath(
            *module_name.split(".")
        ).with_suffix(".py")
        if not module_file.is_file():
            raise FileNotFoundError(
                "DifFlow3D model module was not found: "
                f"{module_file}."
            )
        ...
        if model_class is None:
            raise RuntimeError(
                f"{module_name}.PointConvBidirection was not found."
            )
        ...
        if config.torch_compile_mode != "none":
            ...
            try:
                ...
                raise RuntimeError(
                    "torch.compile failed during model preparation. "
                    "Use --torch-compile-mode none to disable it."
                ) from error
        ...
        if config.execution_backend == "cuda-graph":
            try:
                set_ranges = getattr(
                    self.model_module,
                    "set_profile_ranges_enabled",
                    None,
                )
                if callable(set_ranges):
                    set_ranges(False)

                self.graph_runner = CudaGraphDifFlowRunner(
                    self.model,
                    num_points=self.num_points,
                    uncertainty=config.uncertainty,
                    device=self.device,
                    capture_warmup=config.cuda_graph_warmup,
                )
                self._source_pinned = torch.empty(
                    (1, self.num_points, 3),
                    dtype=torch.float32,
                    pin_memory=True,
                )
                self._target_pinned = torch.empty(
                    (1, self.num_points, 3),
                    dtype=torch.float32,
                    pin_memory=True,
                )
                self._source_pinned_numpy = (
                    self._source_pinned[0].numpy()
                )
                self._target_pinned_numpy = (
                    self._target_pinned[0].numpy()
                )
                self.execution_backend = "cuda-graph"
            except Exception as error:
                self.cuda_graph_error = (
                    f"{type(error).__name__}: {error}"
                )
                self.graph_runner = None
                self._source_pinned = None
                self._target_pinned = None
                self._source_pinned_numpy = None
                self._target_pinned_numpy = None
                if not config.cuda_graph_fallback:
                    raise RuntimeError(
                        "CUDA Graph capture failed and fallback is disabled."
                    ) from error
    ...
    def _extract_state_dict(checkpoint: Any) -> dict[str, torch.Tensor]:
        if not isinstance(checkpoint, dict):
            raise TypeError(
                "Unsupported checkpoint format: expected a state-dict-like "
                f"mapping, received {type(checkpoint).__name__}."
            )
    ...
    def ensure_cuda_graph(self) -> CudaGraphDifFlowRunner:
        ...
        self.graph_runner = CudaGraphDifFlowRunner(
            ...
            device=self.device,
            capture_warmup=self.config.cuda_graph_warmup,
        )
        return self.graph_runner

    def reset_sampling(self) -> None:
        """Reset deterministic pair sampling after warmup."""
        self._pair_counter = 0
    ...
    def _sample_indices(
        ...
        if point_count < 1:
            raise ValueError("Cannot run DifFlow3D on an empty point cloud.")
        return rng.choice(
            point_count,
    ...
    def infer(
        ...
        if self.execution_backend == "cuda-graph":
            if (
                ...
                or self._source_pinned is None
                or self._target_pinned is None
                or self._source_pinned_numpy is None
                or self._target_pinned_numpy is None
            ):
                raise RuntimeError(
                    "CUDA Graph backend was selected but is not initialized."
                )

            # Copy sampled NumPy arrays into reusable page-locked host buffers.
            # H2D copies then execute asynchronously on the current stream,
            # immediately followed by graph replay on that same stream.
            np.copyto(
                self._source_pinned_numpy,
                source_np,
                casting="no",
            ...
            self.graph_runner.copy_pinned_inputs(
                self._source_pinned,
                self._target_pinned,
                inverse_dt=1.0 / dt_s,
            )
            self.graph_runner.replay()

            source_points = self.graph_runner.static_source[0]
            predicted_flow = (
                self.graph_runner.static_predicted_flow
            )
            warped_points = (
                self.graph_runner.static_warped_points
            )
            velocity = self.graph_runner.static_velocity

            if self.config.cuda_graph_clone_outputs:
                # Stable beyond the next replay.  Leave disabled for minimum
                # latency when the caller consumes each estimate immediately.
                source_points = source_points.clone()
                predicted_flow = predicted_flow.clone()
                warped_points = warped_points.clone()
            ...
            # XYZ is also the 3-channel feature for this checkpoint.
            dummy_gt_flow = torch.zeros_like(source)
...
def warmup_from_sequence(
    ...
    *,
    ...
    del point_limit  # Kept only for CLI compatibility with the previous script.
    if repetitions < 1:
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=8192,
        help=(
            "Number of independently sampled source and target points. "
            "The official checkpoint was trained with 8192."
        ),
    )
    parser.add_argument(
        "--difflow-iters",
        type=int,
        default=4,
        help="Recurrent iterations; official evaluation config uses 4.",
    )
    parser.add_argument(
        "--difflow-uncertainty",
        type=float,
        default=0.02,
        help="Official evaluation uncertainty value.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Python module imported from --difflow-repo. Use "
            "model_difflow_profiled for the supplied profiled model file."
        ),
    )
    parser.add_argument(
        "--execution-backend",
        ...
        default="eager",
        help=(
            "Use ordinary eager execution or a static-shape CUDA Graph "
            "captured for [1, --difflow-num-points, 3]."
        ),
    )
    parser.add_argument(
        "--cuda-graph-warmup",
        type=int,
        default=5,
        help="Side-stream forwards executed before CUDA Graph capture.",
    )
    parser.add_argument(
        "--cuda-graph-no-fallback",
        action="store_true",
        help="Fail instead of falling back to eager if graph capture fails.",
    )
    parser.add_argument(
        "--cuda-graph-clone-outputs",
        action="store_true",
        help=(
            "Clone graph outputs before returning them. Safe for retaining "
            "estimates across future replays, but adds copy kernels."
        ),
    )
    parser.add_argument(
        "--torch-compile-mode",
        ...
        default="none",
        help=(
            "Optional torch.compile mode applied before CUDA Graph capture. "
            "Custom PointNet2 operators may cause graph breaks."
        ),
    )
    parser.add_argument(
        "--enable-tf32",
        action="store_true",
        help=(
            "Allow TF32 for float32 matmul/convolution. Faster on supported "
            "GPUs but may slightly change nearest-neighbor tie ordering."
        ),
    )
    parser.add_argument(
        "--disable-cudnn-benchmark",
        action="store_true",
        help="Disable cuDNN algorithm benchmarking for fixed input shapes.",
    )
    parser.add_argument(
        "--profile-cuda",
        action="store_true",
        help="Run a dedicated model-only PyTorch CPU/CUDA profiler pass.",
    )
    parser.add_argument(
        "--profile-only",
        action="store_true",
        help="Exit after profiling instead of running the accuracy benchmark.",
    )
    parser.add_argument(
        "--profile-backend",
        choices=("auto", "eager", "cuda-graph"),
        default="auto",
        help=(
            "Backend profiled by --profile-cuda. auto follows "
            "--execution-backend."
        ),
    )
    parser.add_argument(
        "--profile-output-dir",
        type=Path,
        default=Path("./profiler_difflow3d"),
    )
    parser.add_argument("--profile-source-index", type=int, default=0)
    parser.add_argument("--profile-warmup", type=int, default=10)
    parser.add_argument("--profile-wait", type=int, default=1)
    parser.add_argument("--profile-schedule-warmup", type=int, default=2)
    parser.add_argument("--profile-active", type=int, default=5)
    parser.add_argument("--profile-repeat", type=int, default=1)
    parser.add_argument("--profile-row-limit", type=int, default=80)
    parser.add_argument(
        "--profile-record-shapes",
        action="store_true",
        help="Record and group operator input shapes; adds profiler overhead.",
    )
    parser.add_argument(
        "--profile-memory",
        action="store_true",
        help="Record tensor memory events; adds profiler overhead.",
    )
    parser.add_argument(
        "--profile-with-stack",
        action="store_true",
        help="Record Python stack traces; adds substantial overhead.",
    )
    parser.add_argument(
        "--profile-with-flops",
        action="store_true",
        help="Estimate FLOPs for supported matrix/2D convolution operators.",
    )
...
def main() -> None:
    ...
    if args.profile_only and not args.profile_cuda:
        raise ValueError("--profile-only requires --profile-cuda.")
    ...
    config = DifFlow3DConfig(
        ...
        cuda_graph_fallback=not args.cuda_graph_no_fallback,
    ...
    print(f"Requested backend:     {config.execution_backend}")
    print(f"torch.compile mode:    {config.torch_compile_mode}")
    print(f"TF32 enabled:          {args.enable_tf32}")
    print(
        "cuDNN benchmark:      "
        f"{not args.disable_cudnn_benchmark}"
    )
    ...
    print("Model pair sampling:   independent source/target (NO_CORR=True)")
    print("")
    print("Objects:")
    for spec in OBSTACLE_SPECS:
        print(
    ...
    print(f"Active backend:        {estimator.execution_backend}")
    if estimator.cuda_graph_error is not None:
        print(
            "CUDA Graph fallback:  "
            f"{estimator.cuda_graph_error}"
        )
    ...
    if args.profile_cuda:
        ...
        profile_backend: Literal["eager", "cuda-graph"]
        if args.profile_backend == "auto":
            profile_backend = estimator.execution_backend
        else:
            profile_backend = args.profile_backend

        graph_runner = None
        if profile_backend == "cuda-graph":
            # Nested record_function ranges are executed only during capture,
            # not replay, so disable them to minimize the graph itself.
            if callable(set_ranges):
                set_ranges(False)
            graph_runner = estimator.ensure_cuda_graph()
        else:
            if callable(set_ranges):
                set_ranges(True)
            else:
                print(
                    "Warning: the selected model module does not expose "
                    "set_profile_ranges_enabled(); only operator-level ranges "
                    "will be available."
                )

        profile_source, profile_target = prepare_profile_inputs(
            sequence,
            source_index=args.profile_source_index,
        ...
        print("=" * 80)
        print(f"Profile backend:       {profile_backend}")
        print(f"Profile pair:          {args.profile_source_index:03d}->"
              f"{args.profile_source_index + 1:03d}")
        print(f"Profile input shape:   {tuple(profile_source.shape)}")
        print(f"Profile output dir:    {args.profile_output_dir}")
        ...
        profile_difflow3d(
            ...
            schedule_warmup_steps=args.profile_schedule_warmup,
        ...
        del profile_source, profile_target
        torch.cuda.empty_cache()
