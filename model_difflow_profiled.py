...
neighbor/interpolation maps, reuse fixed diffusion time features, combine
neighbor grouping operations, reduce repeated layout materialization, and
remove dead single-step DDIM work. Training keeps the original execution path,
apart from equivalent broadcast operations.
"""
...
# Evaluation-only optimizations. Each switch can be disabled independently
# for ablation or strict reproduction testing.
#
# Skip FPS when the input already contains exactly all requested level-1
# points. The point set is unchanged; only the full-set FPS permutation is
# replaced by the input ordering.
ENABLE_REDUNDANT_FULL_FPS_BYPASS = True

# Reuse the deterministic t=T-1 diffusion time embedding across the outer
# recurrent iterations of one resolution level.
ENABLE_EVAL_TIME_EMBEDDING_REUSE = True

# Concatenate cosine/spatial neighbor indices first, then group coordinates and
# features once instead of grouping each half separately.
ENABLE_EVAL_COMBINED_NEIGHBOR_GROUPING = True

# Materialize the GRU's channel-first input once before it is consumed by the
# r/z/h convolution branches. This avoids repeated implicit contiguous copies.
ENABLE_EVAL_CONTIGUOUS_GRU_INPUT = True

# Profiling ranges are disabled during normal inference, so the model keeps its
# usual execution path and timing.  The test script enables them only for a
# dedicated profiling run.  This changes no parameters, state_dict keys,
# forward signatures, or returned values.
_PROFILE_RANGES_ENABLED = (
    os.environ.get("DIFFLOW_PROFILE_RANGES", "0") == "1"
)


def set_profile_ranges_enabled(enabled: bool) -> None:
    """Enable or disable nested torch.profiler record_function ranges."""
    global _PROFILE_RANGES_ENABLED
    _PROFILE_RANGES_ENABLED = bool(enabled)


def profile_range(name: str):
    """Return a profiler range only when profiling is explicitly enabled."""
    if _PROFILE_RANGES_ENABLED:
        return record_function(name)
    return nullcontext()


def profile_call(name: str, function, *args, **kwargs):
    """Call a function inside an optional profiler range."""
...
class PointConvEncoder(nn.Module):
    def __init__(self, weightnet=8):
        ...
        # Non-persistent: it follows model.to(device) but does not add a
        # checkpoint/state_dict key. Batch size one, used by this benchmark,
        # can reuse the buffer without launching arange/repeat every forward.
    ...
    def forward(self, xyz, color):
        with profile_range("difflow/encoder/level0"):
        ...
        with profile_range("difflow/encoder/level1_2048"):
            if (
        ...
        with profile_range("difflow/encoder/level2_512"):
        ...
        with profile_range("difflow/encoder/level3_256"):
        ...
        with profile_range("difflow/encoder/level4_64"):
...
class RecurrentUnit(nn.Module):
    ...
    def _prepare_cosine_neighbors(self, feat1, feat2):
        with profile_range("difflow/recurrent/prepare_cosine_knn"):
    ...
    def _prepare_spatial_neighbors(self, pc1, pc2):
        with profile_range("difflow/recurrent/prepare_spatial_knn"):
            half_neighbors = self.flow_nei // 2
    ...
    @staticmethod
    def _prepare_combined_cross_context(
        xyz1,
        xyz2,
        cosine_idx,
        spatial_idx,
        ...
        with profile_range(
            "difflow/recurrent/combine_neighbor_indices"
        ):
            combined_idx = torch.cat(
                (cosine_idx, spatial_idx),
                dim=-1,
            )

        with profile_range(
            "difflow/recurrent/group_combined_geometry"
        ):
            xyz1_n = xyz1.permute(0, 2, 1)
            xyz2_n = xyz2.permute(0, 2, 1)
            neighbor_xyz = index_points_group(
                xyz2_n,
                combined_idx,
            )
            direction_xyz = (
    ...
    @staticmethod
    def _cross_from_combined_context(
        module,
        points1,
        points2,
        combined_idx,
        ...
        with profile_range(
            "difflow/recurrent/group_combined_features"
        ):
        ...
        # Broadcast rather than materializing neighbor_count identical copies.
        grouped_points1 = (
            points1_n.unsqueeze(2)
            .expand(-1, -1, neighbor_count, -1)
            .permute(0, 3, 2, 1)
        )
    ...
    def _bidirectional_fast(
        ...
        feat1_new = self._cross_from_combined_context(
            self.bid,
        ...
        feat2_new = self._cross_from_combined_context(
            self.bid,
    ...
    def _flow_embedding_fast(
        ...
        return self._cross_from_combined_context(
            self.fe,
            points1,
            points2,
            combined_12,
    ...
    def forward(
        ...
        with profile_range("difflow/recurrent/initial_feature_concat"):
            c_feat1 = torch.cat([feat1, feat1_new], dim=1)
            c_feat2 = torch.cat([feat2, feat2_new], dim=1)

        flows = []

        # The training path remains the original implementation. The fast path
        # is evaluation-only, so checkpoint training behavior is intact.
        use_fast_cross = (
            not self.training
        ...
        if use_fast_cross:
            ...
            flow_neighbor_context = profile_call(
                "difflow/recurrent/prepare_diffusion_self_knn",
                self.flow._prepare_neighbor_context,
                pc1,
                pc1,
            )
            if ENABLE_EVAL_TIME_EMBEDDING_REUSE:
                flow_time_per_point = profile_call(
                    "difflow/recurrent/prepare_diffusion_time_embedding",
                    self.flow._prepare_eval_time_per_point,
                    up_flow,
                )
            else:
        ...
        for iteration in range(self.iters):
            with profile_range(
                f"difflow/recurrent/iteration_{iteration}"
            ):
                pc2_warp = profile_call(
                    "difflow/recurrent/warping",
                    self.warping,
                    pc1,
                    pc2,
                    up_flow,
                ...
                if use_fast_cross:
                    spatial_12, spatial_21 = (
                        self._prepare_spatial_neighbors(
                            pc1,
                            pc2_warp,
                        )
                    )
                    if ENABLE_EVAL_COMBINED_NEIGHBOR_GROUPING:
                        context_12 = (
                            self._prepare_combined_cross_context(
                                pc1,
                                pc2_warp,
                                cosine_12,
                                spatial_12,
                        ...
                        feat1_new, feat2_new = profile_call(
                            "difflow/recurrent/bidirectional_cross",
                            self._bidirectional_fast,
                            c_feat1,
                            c_feat2,
                            context_12,
                            context_21,
                        ...
                        fe = profile_call(
                            "difflow/recurrent/flow_embedding",
                            self._flow_embedding_fast,
                            feat1_new,
                            feat2_new,
                            context_12,
                        )
                        ...
                        feat1_new, feat2_new = profile_call(
                            "difflow/recurrent/bidirectional_cross_original",
                            self.bid,
                            pc1,
                            pc2_warp,
                            c_feat1,
                        ...
                        fe = profile_call(
                            "difflow/recurrent/flow_embedding_original",
                            self.fe,
                            pc1,
                            pc2_warp,
                            feat1_new,
                ...
                with profile_range("difflow/recurrent/new_feature_concat"):
                ...
                if self.training:
                    (
                        ...
                        "difflow/recurrent/diffusion_gru_training",
                        self.flow,
                        pc1,
                        pc1,
                        up_feat,
                    ...
                    (
                        ...
                        "difflow/recurrent/diffusion_gru",
                        ...
                        time_per_point=flow_time_per_point,
                    )
                else:
                    feat_flow, flow, certainty_new = profile_call(
                        "difflow/recurrent/diffusion_gru_original",
                        self.flow,
                        pc1,
                        pc1,
                        up_feat,
                ...
                # These concatenations are only consumed by the next
                # iteration.
                if iteration + 1 < self.iters:
                    with profile_range(
                        "difflow/recurrent/next_iteration_concat"
                    ):
                        c_feat1 = torch.cat(
                            [feat1, feat1_new],
                            dim=1,
...
class DiffusionSceneFlowGRUResidual(nn.Module):
    def __init__(self, nsample, in_channel, latent_channel, mlp, mlp2=None, bn=False, use_leaky=True, return_inter=False, radius=None, use_relu=False, channels=[64, 64], clamp=[-200, 200], scale_dif=1.0):
        ...
        # Evaluation-only cache.  The one-step DDIM timestep is always T-1,
        # and time_mlp parameters are immutable in eval mode.  Cache one
        # expanded time embedding per (batch, point_count, device, dtype).
        # This is a normal Python attribute, not a buffer/parameter, so it
        # introduces no checkpoint keys and is cleared when training resumes.
    ...
    def _prepare_neighbor_context(self, xyz1, xyz2):
        ...
        with profile_range("difflow/diffusion_gru/self_knn"):
    ...
    def _prepare_eval_time_per_point(self, flow):
        ...
        device_index = (
            int(flow.device.index)
            if flow.device.index is not None
        ...
        with profile_range("difflow/diffusion_gru/time_embedding_cache_miss"):
            t = torch.full(
                (batch_size,),
                self.timesteps - 1,
                device=flow.device,
            ...
            cached = self.time_mlp(t).unsqueeze(1).expand(
                -1,
                point_count,
                -1,
            )
    ...
    def _gru_update_eval(
        ...
        if ENABLE_EVAL_CONTIGUOUS_GRU_INPUT:
            # r, z and h all consume this tensor. One explicit materialization
            # is cheaper than allowing each convolution path to request its
            # own implicit contiguous copy.
            with profile_range(
                "difflow/diffusion_gru/materialize_channel_first_input"
            ):
    ...
    def _forward_eval_fast(
        ...
        # only pair is (T-1, -1). The time value is deterministic and may be
        # prepared once by RecurrentUnit and reused across its outer iterations.
        ...
        with profile_range("difflow/diffusion_gru/random_noise"):
            delta_flow = (
        ...
        for _ in range(self.iters):
            new_points, delta_flow, delta_certainty = profile_call(
                "difflow/diffusion_gru/gated_update",
                self._gru_update_eval,
                points1,
                points2,
                delta_flow.detach(),
...
class PointConvBidirection(nn.Module):
    ...
    def _prepare_upsample_context(xyz, sparse_xyz):
        ...
        with profile_range("difflow/upsample/prepare_3nn"):
    ...
    def _apply_upsample_context(sparse_values, context):
        with profile_range("difflow/upsample/apply_3nn"):
            ...
            # Match UpsampleFlow's original non-contiguous permuted view.
    ...
    def forward(self, xyz1, xyz2, color1, color2, gt_flow, uncertainty=0.5):
        ...
        pc1s, feat1s, idx1s = profile_call(
            "difflow/encoder_source",
            self.encoder,
            xyz1,
            color1,
        )
        pc2s, feat2s, idx2s = profile_call(
            "difflow/encoder_target",
            self.encoder,
            xyz2,
            color2,
        )
        ...
        feat1_new_l3, feat2_new_l3, cross3 = profile_call(
            "difflow/coarse_l3/cross",
            self.cross3,
            pc1s[3],
            pc2s[3],
            c_feat1_l3,
        ...
        feat3, flow3, certainty3 = profile_call(
            "difflow/coarse_l3/flow_estimator",
            self.flow3,
            pc1s[3],
            feat1s[3],
            cross3,
